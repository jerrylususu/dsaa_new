...
// it seems have something to do with lambda expression
// change lambda expression into normal comparator, and the time gets normal
// why? lambda expression needs prepare time!
// see: https://stackoverflow.com/questions/34585444/java-lambdas-20-times-slower-than-anonymous-classes

// sphere might be overlapping!
// use math.max to solve that

// still wa
// found a missing long in dijk1
// caused inaccuracy
// moved and good
...
//                out.printf("%d\n",Math.round(dist[holen+1]));
                out.println(Math.round(dist[holen+1]));
            }
    ...
    public static double weight(Hole[] arr, int i, int j){
        return Math.max(
                100*(
                        Math.sqrt(
                                pow(arr[i].x-arr[j].x,2)+pow(arr[i].y-arr[j].y,2)+pow(arr[i].z-arr[j].z,2))
                                - arr[i].r - arr[j].r),0);
    }
    ...
    public static double[] dijk(MyNode s,MyNode[] arr){
        ...
        PriorityQueue<Pair> pq = new PriorityQueue<>((o1, o2) -> Double.compare(o1.dist,o2.dist));
...
//        });
    ...
    public static double[] dijk2(MyNode s,MyNode[] arr){
        int len = arr.length;
        double[] dist = new double[len];
        Arrays.fill(dist,-1);
...
//        vis[s.id]=true;
        ...
        while(checked<len){
            ...
            // extend on that min
            curnode = arr[curminid];
            checked++;
//            vis[curminid] = true;
            for(MyEdge e:curnode.edges){
                int opid = e.sum - curnode.id;
                if(dist[opid]==-1||dist[curminid] + e.weight < dist[opid]){
                    dist[opid] = dist[curminid] + e.weight;
                }
            }
        }
