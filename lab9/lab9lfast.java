    ...
    static class Task {
        ...
        public void solve(InputReader in2, PrintWriter out) throws IOException {
            ...
            while(totalnum-->0){
                ...
                MyEdge[] edges = new MyEdge[edgeset.size()];
                int iternum = 0;
...
//                System.out.println(sccs);
                ...
                // do point combine
                MyNode[] combined = new MyNode[sccs.size()];
                ...
                for(List<MyNode> li:sccs){
                    ...
                    MyNode combnode = new MyNode(noden+iternum);
                    combined[iternum++] = combnode;
                    for(MyNode curnode:li){
                        for(MyEdge e:curnode.in){
                            if(!e.vis){
                                if(!setcheck[e.from]){
                                    // from outside
                                    // move this edge into the combined node
//                                e.to = combnode.id; // warning:sum not updated
                                    combnode.inn++;
                                } else {
                                    // in scc
                                    // just eliminate this node
                                    nodes[e.from].outn--;
                                    nodes[e.to].inn--;
                                    e.vis=true;
                                }
                ...
                // do calculation
                int noincnt=0, nooutcnt=0, nobothcnt=0;
                for(MyNode n:nodes){
                    if(n.enable){
                        if(n.inn==0&&n.outn==0) nobothcnt++;
                        else if(n.inn==0) noincnt++;
                        else if(n.outn==0) nooutcnt++;
                    }
                }
